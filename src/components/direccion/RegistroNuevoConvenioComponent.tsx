...
export const RegistroNuevoConvenio = () => {
  ...
  const validar = ({ nit, fecha, file }: any) => { 
  ...
  const onSubmit = (e: any) => { 
    e.preventDefault()
    const { nit, fecha } = e.target
    console.log('convenio file', file?.type)
    const errorEncontrado = validar({ nit: nit.value, fecha: fecha.value, file: file?.type })
    if (errorEncontrado) { 
      return
    }
    console.log('enviando...', nit.value, fecha.value)
  }
